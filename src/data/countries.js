// Country dial codes for the phone fields.
//
// Kept in step with HitchLink_frontend/src/data/countries.js by hand — the two
// apps are separate repos with no shared package, so this is a copy. If a
// country is added there, add it here too.
//
// Keyed by ISO 3166-1 alpha-2 because dial codes are NOT unique — US/CA both
// use +1 and RU/KZ both use +7, so the ISO code is the only safe form value.
// `dial` is digits only (no "+"); the UI adds the plus.
// Flags are rendered from `code` via flagcdn (see CountrySelect), so there are
// no flag assets to keep in sync here.
...
export const COUNTRIES = [
    ...
    { code: "VI", name: "Virgin Islands (U.S.)",  dial: "1340" },
...
export const countryByCode = (code) => BY_CODE.get(code) ?? BY_CODE.get("US");
